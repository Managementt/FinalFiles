...
const CLIENT_ID = "992160036280-eqdq9aivb6ge2k7m4hbj45drnpsuj8mf.apps.googleusercontent.com";
...
const columns = [
  "Timestamp", "FileType", "FileStatus", "BookDetails", "BIN",
  "Book Cover PDF", "Book Cover PSD", "Cover Front Thumbnail", "Cover Back Thumbnail",
  "Book Cover Inside PDF (if applicable)", "Book Cover Inside PSD (if applicable)",
  "Cover Mockups", "Mockup 1", "Mockup 2", "Mockup 3",
  "Cover for GDP - PDF", "Cover for GDP - PSD",
  "Upload PDF File", "Upload Open File (Indesign/Word/PageMaker/.pages)",
  "Interior File for GDP", "Upload EPUB file", "Upload KPF file",
  "Upload the poster PDF", "Revised Mockup", "Interior File for GDP"
];
...
const UploadForm = () => {
  ...
  const fileFieldToColumn = {
    ...
    "Book Cover Inside PDF (if applicable)": "Book Cover Inside PDF (if applicable)",
    "Book Cover Inside PSD (if applicable)": "Book Cover Inside PSD (if applicable)",
    ...
    "Upload Open File (Indesign/Word/PageMaker/.pages)": "Upload Open File (Indesign/Word/PageMaker/.pages)",
      ...
      if (url && fileFieldToColumn[fieldName]) {
        ...
        if (fileType === "posterFile") {
          if (columnName === "Cover for GDP - PDF") {
            ...
            uploadedUrls["Upload Open File (Indesign/Word/PageMaker/.pages)"] = url;
          }
        }
      } else {
  ...
  const sections = {
    coverFile: [
      "Book Cover PDF", "Book Cover PSD", "Cover Front Thumbnail", "Cover Back Thumbnail",
      "Book Cover Inside PDF (if applicable)", "Book Cover Inside PSD (if applicable)",
      "Cover Mockups", "Mockup 1", "Mockup 2", "Mockup 3", "Cover for GDP - PDF", "Cover for GDP - PSD"
    ],
    interiorFile: [
      "Upload PDF File", "Upload Open File (Indesign/Word/PageMaker/.pages)", "Interior File for GDP"
    ],
    epubFile: ["Upload EPUB file", "Upload KPF file"],
    posterFile: [
      "Upload the poster PDF", "Cover for GDP - PDF", "Cover for GDP - PSD",
      "Revised Mockup", "Interior File for GDP"
    ],
  };

  return (
    <div>
      <h2>Upload Files to Wasabi & Save URLs in Google Sheets</h2>
      <form onSubmit={handleSubmit}>
        ...
        <label>
          BIN:
          <input type="text" name="bin" value={formData.bin || ""} onChange={handleChange} required placeholder="Enter BIN" />
        </label>

        <br />
        <label>
          File Status:
          <input type="text" name="fileStatus" value={formData.fileStatus || ""} onChange={handleChange} placeholder="Enter file status" />
        </label>

        <br />
        <label>
          Book Details:
          <input type="text" name="bookDetails" value={formData.bookDetails || ""} onChange={handleChange} placeholder="Enter book details" />
        </label>
